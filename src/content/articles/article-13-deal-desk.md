---
title: "How to Build a Well-Designed Deal Desk that Speeds Deals Up."
description: "Build discount governance and exception handling into your CPQ from the start to unlock speed and control at the same time."
date: 2026-06-02
...
Most deal desks don't work that way. They slow deals down because the approvals are cumbersome, the boundaries are unclear, and the process is manual. This results in reps treating the desk as an obstacle rather than a resource. That's a design problem and it has a design solution.

## Define What Reps Can Do Without Approval

The most important input to deal desk design is a clear, documented discount authority matrix which shows what each seller level can approve without escalation, by product type, deal size, and customer segment. When this doesn't exist, every non-standard deal goes to the desk regardless of whether it actually needs review. Then volume increases and response times slow.
...
**Design the discount authority matrix before you configure approval workflows.** The system can enforce whatever rules you define but the rules have to come from a business decision about where your discount authority actually sits. Get product management, sales leadership, finance, and legal in a room to agree on the matrix and document it. That document becomes the foundation for your approval configuration.

## Build the Governance Into CPQ, Not Around It

The failure mode for most deal desks is that the governance layer sits outside the CPQ system in a separate Slack channel, an email thread, or a spreadsheet the desk maintains. Reps work around the system because the system doesn't know what to do with non-standard deals, so they email someone and wait.

Agentforce Revenue Managament can enforce your discount authority matrix natively. Quotes that fall within rep authority move forward without friction. Quotes that exceed thresholds automatically route to the right approver with the context they need to make a decision: deal size, margin impact, customer history, what the rep is asking for and why. The approver gets a single-click decision interface, not an email chain to decipher.

**Configure your approval logic to match the actual decision hierarchy.** This means mapping your authority matrix into CPQ approval rules with threshold-based routing, parallel approvals for multi-party decisions, and escalation paths when response times exceed SLA. When it's done well, the system handles the routing and the deal desk handles the judgment calls, which is what the desk should actually be doing.
...
The information that belongs in every deal desk submission: current contract terms with the customer, discount history on this account, margin at the proposed price, competitive context if it's relevant, and what the rep is specifically requesting and why. Agentforce Revenue Management can surface most of this automatically from your Salesforce data like account history, prior quote data, contract records, etc. 
...
Publish a response time commitment like 4 hours for standard exceptions, 24 hours for complex deals over a certain threshold, or same-day for end-of-quarter urgency with proper flagging. Then keep that commitment. Track response time as a metric. Review it in sales operations meetings. When the desk misses SLA, understand why and fix it.
...
A high volume of deal desk submissions is a signal worth investigating. When a majority of deals require non-standard pricing, the problem is likely in the standard pricing. It's too high for the market, not structured in a way that maps to how customers actually want to buy, or missing discount tiers that effectively reflect normal deal economics.
...
A well-calibrated deal desk handles genuine exceptions like large strategic deals, unusual commercial structures, and competitive situations that require creativity. It should not be the primary path for normal enterprise deals that happen to fall slightly outside a pricing model that hasn't been updated in two years.

---
*I work on ARM and Q2C implementations at Slalom. If you're designing or rebuilding your deal desk process and want to talk through the approach, connect with me on [LinkedIn](https://linkedin.com/in/carlosali).*
